...
create_directories() {
    ...
    # Zsh: add to .zshrc if it exists
    if [[ -f "$HOME/.zshrc" ]] && ! grep -q 'local/bin' "$HOME/.zshrc"; then
        echo 'export PATH="$HOME/.local/bin:$PATH"' >> "$HOME/.zshrc"
    fi

    # Fish: fish_add_path is idempotent — safe to run even if already set
...
EOF
    ...
    echo -e "  ${CYAN}(Re-run options 9/10 to regenerate full shell configs with OMP baked in.)${NC}"
...
install_zsh() {
    ...
    # ── 2. Install zsh-autosuggestions & zsh-syntax-highlighting via DNF ─
    ...
    # ── 5. Install thefuck (command correction) ────────────────────────
...
install_fish() {
    ...
    # fzf.fish — Ctrl+R history, Ctrl+T file search, Alt+C dir jump
    ...
    # sponge — clean history of failed/typo commands
    ...
    # tide — fast, informative prompt (CachyOS uses it)
    ...
    # ── 5. Write ~/.config/fish/config.fish ────────────────────────────
...
# ── fzf options (Catppuccin Macchiato palette) ────────────────────
...
# ── thefuck (Esc Esc to fix last command) ─────────────────────────
...
FISH_CONTENT
    ...
    # ── 6. Write fish abbreviations (like aliases but smarter) ──────────
    print_section "Setting up fish abbreviations"
    cat > "$HOME/.config/fish/conf.d/abbr.fish" << 'ABBR_CONTENT'
# Fish abbreviations (expand inline as you type)
abbr --add dnfi  'sudo dnf install'
abbr --add dnfr  'sudo dnfu remove'
...
ABBR_CONTENT
    ...
    # ── 7. Tide prompt: auto-configure with nice defaults ───────────────
...
show_menu() {
    ...
    echo "  12) Install Winboat --- Running windows natively currently in beta"
    echo ""
    echo "  -- Drivers & Apps --"
    echo "  13) Install NVIDIA Drivers"
    echo "  14) Install AppImages (Helium, Capacities, Affinity Installer)"
    echo ""
    echo "  -- Security --"
    ...
    echo "  18) Privacy & Network Tools (Tor, MAC spoof, WireGuard, DNS-DoT)"
    echo ""
    echo "  -- Storage --"
    ...
    echo ""
    echo "  -- Privacy Extras --"
    echo "  21) Harden Browser Privacy (Firefox / LibreWolf / Zen Browser)"
    echo ""
    echo "  -- Hacking Tools --"
    echo "  22) Hacking Tools -- Kali Linux Toolkit"
    echo ""
    echo "  -- Hyprland --"
    echo "  23) Install Hyprland + Dotfiles (JaKooLit / ML4W / end-4)"
    echo ""
    echo "  0)  Exit"
    echo ""
...
install_everything() {
    ...
    echo -e "    3. Run option 9 to set up Zsh (CachyOS-style)"
    echo -e "    4. Run option 10 to set up Fish Shell (CachyOS-style)"
